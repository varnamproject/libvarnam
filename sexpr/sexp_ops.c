...
sexp_t *copy_sexp(const sexp_t *s) {
  ...
  if (s_new->ty == SEXP_VALUE) {
	...
	return NULL;
      ...
      s_new->val_used = s_new->val_allocated = s->val_used;
	...
	/** allocate space **/
#ifdef __cplusplus
	s_new->val = (char *)sexp_malloc(sizeof(char)*s->val_used);
#else
	s_new->val = sexp_malloc(sizeof(char)*s->val_used);
#endif
